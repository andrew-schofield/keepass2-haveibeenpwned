...
namespace HaveIBeenPwned.UI {
    public partial class BreachedEntriesDialog : Form {
        ...
        private void RefreshBreaches() {
            ...
            this.Text = string.Format(this.Text, _breaches.Count, _breaches.Count > 1 ? "Entries" : "Entry");

            breachedEntryList.Items.Clear();
            breachedEntryList.Groups.Clear();
            breachedEntryList.SmallImageList = new ImageList();
            var groupNames = _breaches.Where(b => b.Entry != null).Select(b => b.Entry.ParentGroup.GetFullPath(" - ", false)).Distinct();
            ...
            foreach(var breach in _breaches) {
                ...
                var newItem = new ListViewItem(new[]
                {
                    ...
                    breach.Entry != null ? (chkHidePassword.Checked ? "********" : breach.Entry.Strings.ReadSafe(PwDefs.PasswordField)) : string.Empty,
        ...
        public BreachedEntriesDialog(IPluginHost pluginHost, IList<BreachedEntry> breaches) {
                ...
                if(entry != null) {
                    ...
                    var thread = new Thread(() => {
                        if(pwForm.ShowDialog() == DialogResult.OK) {
                            ...
                            breachedEntryList.SelectedItems[0].SubItems[2] = new ListViewItem.ListViewSubItem(breachedEntryList.SelectedItems[0], (chkHidePassword.Checked ? "********" : entry.Strings.ReadSafe(PwDefs.PasswordField)));
                            breachedEntryList.SelectedItems[0].SubItems[3] = new ListViewItem.ListViewSubItem(breachedEntryList.SelectedItems[0], entry.Strings.ReadSafe(PwDefs.UrlField));
                            breachedEntryList.SelectedItems[0].SubItems[4] = new ListViewItem.ListViewSubItem(breachedEntryList.SelectedItems[0], entry.GetPasswordLastModified().ToShortDateString());
                        } else {
                            bool bUpdImg = pluginHost.Database.UINeedsIconUpdate;
